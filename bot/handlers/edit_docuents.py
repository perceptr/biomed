...
from bot.keyboards.edit_docuemnts_kb import kb_edit_document
from bot.keyboards.list_documents_kb import kb_list_edit_documents
...
async def handle_edit_doc_text(call: CallbackQuery, state: FSMContext):
    await call.answer()
    await call.message.answer('Пожалуйста, укажите, что именно нужно исправить в расшифровке.')
...
async def handle_delete_doc(call: CallbackQuery, state: FSMContext):
    await call.answer()
    await call.message.answer('Вы действительно хотите удалить расшифровку анализа? Напишите "Удалить"')
