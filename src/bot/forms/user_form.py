...
@user_info_router.callback_query(
    (F.data == "register_user"), ~HasReadPrivacyPolicyFilter()
)
...
@user_info_router.callback_query(
    (F.data == "register_user") | (F.data == "edit_user"), HasReadPrivacyPolicyFilter()
)
...
async def capture_name(message: Message, state: FSMContext):
    ...
    async with ChatActionSender.typing(bot=bot, chat_id=message.chat.id):
        await message.answer("Введите пол:", reply_markup=kb_sex())
...
async def capture_year_of_birth(message: Message, state: FSMContext):
    ...
    if not validate_year_of_birth(year_of_birth):
        await message.reply(
            "Пожалуйста, введите корректный год рождения (от 1900 до текущего)."
        )
        return
